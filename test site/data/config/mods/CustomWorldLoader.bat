...
cls
TITLE Loading internal world...
echo.
echo.
echo Fetching the pre-loaded custom world...
echo.
echo.
...
:WORLDCHECKLIST
IF EXIST world.bat goto WORLDLOAD
cls

echo Failed to detect a local world to load!
echo.
echo.
echo Exiting back to the Game in 5 seconds...
echo.
...
echo.
echo.
echo Detected a local world! Would you like to enter this custom world? [Y/N]
echo.
echo.

set /p "cho=> "

IF "%cho%"=="Y" goto STARTCWL
IF "%cho%"=="y" goto STARTCWL
IF "%cho%"=="N" goto CANCELCWL
IF "%cho%"=="n" goto CANCELCWL
goto WORLDLOAD
...
echo.
echo.
echo Cancelling transfer services...
echo.
echo.
echo Returing to the Main Menu in 5 seconds...
echo.
TIMEOUT 4 /NOBREAK >NUL
goto ENDOFFILE
